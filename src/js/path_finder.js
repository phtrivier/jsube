...
PathFinder.prototype.update_path = function(puzzle, new_goal) {
    if (this.goal_changed(new_goal)) {
	this.goal = new_goal;
	puzzle.clear_path();
	find_path(puzzle, puzzle.player, this.goal, puzzle.current_move_type)
    }
}
...
PathFinder.prototype.find_path = function(puzzle, start, end, move_type) {
    ...
    var expected_goal = new Node(end)
	...
	if (current.equals(expected_goal)) {
		...
		if (!opened.visited(successor)) {
		    agenda.put(successor, successor.estimated_cost(expected_goal));
